...
function InputController:Init()
	...
	-- Prefer showing touch chrome whenever TouchEnabled (phones/tablets); hide on pure desktop
	...
	-- Studio device emulator: TouchEnabled true
	...
	UserInputService.InputChanged:Connect(function(input, processed)
		...
		elseif input.UserInputType == Enum.UserInputType.Touch then
			-- Look handled via touch look zone
		end
	end)
end
...
function InputController:_bindKeyboard()
	...
	-- Also bind Jump via CAS for gamepad
...
function InputController:_buildTouchGui()
	...
	-- Large hit targets; keep center clear for gameplay
	local fire = self:_makeButton(gui, "Fire", "FIRE", UDim2.new(1, -90, 1, -120), UDim2.fromOffset(110, 110))
	self:_bindHold(fire, "Fire")

	local jump = self:_makeButton(gui, "Jump", "JUMP", UDim2.new(1, -90, 1, -250), UDim2.fromOffset(90, 90))
	self:_bindHold(jump, "Jump")

	local crouch = self:_makeButton(gui, "Crouch", "SLIDE", UDim2.new(1, -200, 1, -110), UDim2.fromOffset(90, 90))
	self:_bindHold(crouch, "Crouch")

	local ability = self:_makeButton(gui, "Ability", "ABILITY", UDim2.new(1, -200, 1, -230), UDim2.fromOffset(90, 90))
	ability.BackgroundColor3 = Color3.fromRGB(40, 80, 120)
	self:_bindHold(ability, "Ability")

	local reload = self:_makeButton(gui, "Reload", "R", UDim2.new(1, -300, 1, -110), UDim2.fromOffset(70, 70))
	...
	local cycle = self:_makeButton(gui, "Cycle", "GUN", UDim2.new(1, -300, 1, -200), UDim2.fromOffset(70, 70))
	...
	-- Look zone (right half, but not covering buttons) — drag to look
	local lookZone = Instance.new("Frame")
	lookZone.Name = "LookZone"
	lookZone.BackgroundTransparency = 1
	lookZone.Size = UDim2.new(0.55, 0, 0.55, 0)
	lookZone.Position = UDim2.new(0.45, 0, 0, 0)
	lookZone.Parent = gui
